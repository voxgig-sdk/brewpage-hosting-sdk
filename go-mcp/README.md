# brewpage-hosting-mcp

[MCP](https://modelcontextprotocol.io) server exposing the BrewpageHosting SDK as
two agent tools — `brewpage-hosting_list` and `brewpage-hosting_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/brewpage-hosting-mcp)
make build

# 2. Provide credentials via the environment
export BREWPAGE_HOSTING_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user brewpage-hosting \
  -- /absolute/path/to/brewpage-hosting-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./brewpage-hosting-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// brewpage-hosting_list: first page of records
{ "entity": "stat" }
{ "entity": "stat", "query": { } }

// brewpage-hosting_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/brewpage-hosting-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user brewpage-hosting \
     -- "$PWD"/dist/*/brewpage-hosting-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `brewpage-hosting_list` and `brewpage-hosting_load` tools now appear
   in new sessions. Ask the agent to *"list stat using brewpage-hosting"*
   and it calls `brewpage-hosting_list` with `{"entity":"stat"}`.
...
```sh
export BREWPAGE_HOSTING_APIKEY=sk_live_xxx            # API key
export BREWPAGE_HOSTING_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `brewpage-hosting_list` tool
...
### Call the `brewpage-hosting_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `brewpage-hosting_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `brewpage-hosting_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BREWPAGE_HOSTING_APIKEY` | API key sent with every request. |
| `BREWPAGE_HOSTING_BASE` | Optional override of the API base URL. |
...
```sh
./brewpage-hosting-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"brewpage-hosting_load","arguments":{"entity":"stat","query":{"id":1}}}}'
```
