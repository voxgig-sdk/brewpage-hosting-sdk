...
const __1 = require("..");
const SDK_NAME = 'BrewpageHostingSDK';
...
const TEST_SEED = { "entity": { "stat": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+BrewpageHostingSDK\b/.test(code) ||
        /\bBrewpageHostingSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+BrewpageHostingSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/BrewpageHostingSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
