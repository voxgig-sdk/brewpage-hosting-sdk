// Command BrewpageHosting-cli is a CLI + REPL for the BrewpageHosting SDK,
...
const prompt = "brewpage-hosting"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "stat"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      BREWPAGE_HOSTING_APIKEY (api key), BREWPAGE_HOSTING_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: BREWPAGE_HOSTING_APIKEY carries the API key and
	// BREWPAGE_HOSTING_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("BREWPAGE_HOSTING_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("BREWPAGE_HOSTING_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewBrewpageHostingSDK(opts)
