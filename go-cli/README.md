# brewpage-hosting-cli

boru-driven command-line client **and** interactive REPL for the BrewpageHosting
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/brewpage-hosting-cli)
make build

# 2. See usage (words, entities, env vars)
./brewpage-hosting-cli --help

# 3. Provide credentials once, via the environment
export BREWPAGE_HOSTING_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./brewpage-hosting-cli load 1 stat            # {id:1} shorthand
./brewpage-hosting-cli load '{id:1}' stat       # explicit match map

# 5. Override the API base URL for a single call
BREWPAGE_HOSTING_BASE=https://api.example.com ./brewpage-hosting-cli load 1 stat

# 6. No arguments -> interactive REPL
./brewpage-hosting-cli
brewpage-hosting> load 1 stat
brewpage-hosting> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/brewpage-hosting-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/brewpage-hosting-cli load 1 stat
   ```
...
```sh
./brewpage-hosting-cli load 1 stat          # scalar shorthand for {id:1}
./brewpage-hosting-cli load '{id:1}' stat     # explicit match map
```
...
./brewpage-hosting-cli load 1 stat
```
...
Run with no arguments to open a REPL (prompt `brewpage-hosting>`). Each line is
evaluated as its own boru expression:

```text
$ ./brewpage-hosting-cli
brewpage-hosting> load 1 stat
brewpage-hosting> /help
brewpage-hosting> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `stat`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/brewpage-hosting-cli`. |
...
(`./brewpage-hosting-cli <expr>`) and interactively (the REPL), and expressions compose the
