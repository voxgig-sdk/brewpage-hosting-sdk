...
module BrewpageHostingUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = BrewpageHostingResponse.new({})
    ...
    if fetch_err
      ...
      response = BrewpageHostingResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = BrewpageHostingResponse.new(fetched)
    else
