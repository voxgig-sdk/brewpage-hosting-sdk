    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BREWPAGE_HOSTING_TEST_STAT_ENTID JSON to run live")
        client = setup["client"]
...
def _stat_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("BREWPAGE_HOSTING_TEST_STAT_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("BREWPAGE_HOSTING_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
            },
            extra or {},
        ])
        client = BrewpageHostingSDK(helpers.to_map(merged_opts))

    _live = env.get("BREWPAGE_HOSTING_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("BREWPAGE_HOSTING_TEST_EXPLAIN") == "TRUE",
        "live": _live,
