# BrewpageHosting SDK test runner
...
class BrewpageHostingTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            BrewpageHostingTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = BrewpageHostingTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = BrewpageHostingTestRunner.getenv("BREWPAGE_HOSTING_TEST_LIVE")
        override = BrewpageHostingTestRunner.getenv("BREWPAGE_HOSTING_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = BrewpageHostingTestRunner.getenv(key)
        ...
        explain = BrewpageHostingTestRunner.getenv("BREWPAGE_HOSTING_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if BrewpageHostingTestRunner._test_control is not None:
            return BrewpageHostingTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                BrewpageHostingTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            BrewpageHostingTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = BrewpageHostingTestRunner.load_test_control()
...
def load_env_local():
    BrewpageHostingTestRunner.load_env_local()


def env_override(m):
    return BrewpageHostingTestRunner.env_override(m)


def entity_data(v):
    return BrewpageHostingTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return BrewpageHostingTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return BrewpageHostingTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return BrewpageHostingTestRunner.load_test_control()


def live_client_options():
    return BrewpageHostingTestRunner.live_client_options()


def live_delay_ms():
    return BrewpageHostingTestRunner.live_delay_ms()
