# BrewpageHosting SDK Stat entity
...
class StatEntity
  ...
  def data_set(args)
    if args
      @_data = BrewpageHostingHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = BrewpageHostingHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Stat, Hash] the loaded Stat; raises BrewpageHostingError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = BrewpageHostingHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
