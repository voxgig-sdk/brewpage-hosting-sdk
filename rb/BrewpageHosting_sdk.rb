...
require_relative 'BrewpageHosting_types'
...
class BrewpageHostingSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = BrewpageHostingHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = BrewpageHostingHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, BrewpageHostingFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    BrewpageHostingUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = BrewpageHostingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = BrewpageHostingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = BrewpageHostingHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => BrewpageHostingError.new(
        "#{op}_allow",
        "BrewpageHostingSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue BrewpageHostingError => err
    ...
    if fetched.is_a?(Hash)
      status = BrewpageHostingHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = BrewpageHostingError.new(
        "graphql_error", "BrewpageHostingSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = BrewpageHostingSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
