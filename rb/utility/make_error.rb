...
module BrewpageHostingUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = BrewpageHostingContext.new({}, nil)
    end
    op = ctx.op || BrewpageHostingOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || BrewpageHostingResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(BrewpageHostingError) ? err.msg : err.to_s
    ...
    sdk_err = BrewpageHostingError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(BrewpageHostingError)
