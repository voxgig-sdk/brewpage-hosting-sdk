...
import (
	...
	omni "github.com/voxgig-sdk/brewpage-hosting-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.BrewpageHostingError); is {
				out := map[string]any{
					"name":    "BrewpageHostingError",
...
func omniCtx(arg any, client *sdk.BrewpageHostingSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.BrewpageHostingSDK); is {
			client = live
