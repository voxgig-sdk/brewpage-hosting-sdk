...
// BrewpageHosting SDK retry feature
...
class BrewpageHostingRetryFeature extends BrewpageHostingBaseFeature
{
    ...
    public function init(BrewpageHostingContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (BrewpageHostingContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(BrewpageHostingContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(BrewpageHostingContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
