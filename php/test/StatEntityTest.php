...
class StatEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set BREWPAGEHOSTING_TEST_STAT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function stat_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("BREWPAGEHOSTING_TEST_STAT_ENTID");
    $idmap_overridden = $entid_env_raw !== false && str_starts_with(trim($entid_env_raw), "{");

    $env = Runner::env_override([
        "BREWPAGEHOSTING_TEST_STAT_ENTID" => $idmap,
        "BREWPAGEHOSTING_TEST_LIVE" => "FALSE",
        "BREWPAGEHOSTING_TEST_EXPLAIN" => "FALSE",
    ]);

    $idmap_resolved = Helpers::to_map(
        $env["BREWPAGEHOSTING_TEST_STAT_ENTID"]);
    if ($idmap_resolved === null) {
        $idmap_resolved = Helpers::to_map($idmap);
    }

    if ($env["BREWPAGEHOSTING_TEST_LIVE"] === "TRUE") {
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["BREWPAGEHOSTING_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
