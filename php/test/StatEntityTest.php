...
class StatEntityTest extends TestCase
{
    ...
    {
        $testsdk = BrewpageHostingSDK::test(null, null);
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set BREWPAGE_HOSTING_TEST_STAT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function stat_basic_setup($extra)
{
    ...
    $client = BrewpageHostingSDK::test($options, $extra);
    ...
    $entid_env_raw = getenv("BREWPAGE_HOSTING_TEST_STAT_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["BREWPAGE_HOSTING_TEST_STAT_ENTID"]);
    if ($idmap_resolved === null) {
        $idmap_resolved = Helpers::to_map($idmap);
    }

    if ($env["BREWPAGE_HOSTING_TEST_LIVE"] === "TRUE") {
        $merged_opts = Vs::merge([
            [
            ],
            $extra ?? [],
        ]);
        $client = new BrewpageHostingSDK(Helpers::to_map($merged_opts));
    }
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["BREWPAGE_HOSTING_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
