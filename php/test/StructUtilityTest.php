...
// BrewpageHosting SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = BrewpageHostingOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(BrewpageHostingOmni::structfix($now, Struct::undef()))
                        !== json_encode(BrewpageHostingOmni::structfix($val, Struct::undef()))) {
