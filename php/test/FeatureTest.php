...
// BrewpageHosting SDK feature test
//
...
class FtCtrl extends BrewpageHostingControl
{
...
class FtClient
{
    ...
    private BrewpageHostingUtility $utility;

    public function __construct(BrewpageHostingUtility $utility, array $options)
    {
    ...
    public function get_utility(): BrewpageHostingUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (BrewpageHostingContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public BrewpageHostingContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (BrewpageHostingContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = BrewpageHostingConfig::make_feature($name);
    ...
    private static function buildUrl(BrewpageHostingSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof BrewpageHostingError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new BrewpageHostingResponse($fetched);
        }
    ...
    private function fail(BrewpageHostingContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (BrewpageHostingContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
