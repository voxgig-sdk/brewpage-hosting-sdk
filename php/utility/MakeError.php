...
class BrewpageHostingMakeError
{
    public static function call(?BrewpageHostingContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new BrewpageHostingContext([], null);
        }
        $op = $ctx->op ?? new BrewpageHostingOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new BrewpageHostingResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof BrewpageHostingError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new BrewpageHostingError('', $msg, $ctx);
        ...
        if ($err instanceof BrewpageHostingError) {
